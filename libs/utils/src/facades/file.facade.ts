...
export class FileFacade {
  ...
  async addMetadata(
    ...
    const payloads = responses.map((response) => {
      const successResponse =
        response as SuccessResponse<CreateFileMetadataCommand.ResponsePayload>;
  ...
  async findById(id: number, traceId: string) {
    ...
    const fileResponse =
      await this.amqpConnection.request<FindFileByIdQuery.Response>({
    ...
    const successResponse =
      fileResponse as SuccessResponse<FindFileByIdQuery.ResponsePayload>;
  ...
  async findByProviderId(id: string, traceId: string) {
    ...
    const fileResponse =
      await this.amqpConnection.request<FindFileByProviderIdQuery.Response>({
    ...
    const successResponse =
      fileResponse as SuccessResponse<FindFileByProviderIdQuery.ResponsePayload>;
