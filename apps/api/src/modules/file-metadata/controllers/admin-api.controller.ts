...
import { CreateFileMetadataCommand, DeleteFileMetadataCommand, FindFileMetadataByFileIdQuery } from '@app/contracts'
import { TraceId } from "@app/logger";
import { IsStringNumberPipe, Roles } from "@app/utils";
import { Role } from "@app/types";
import { CreateFileMetadataBodyDto, FindFileMetadataByFileIdBodyDto } from "@app/validation";
...
export class AdminApiController {
  ...
  @Get('/')
  @Roles(Role.ADMIN)
  findByFileId(@TraceId() traceId: string | undefined, @Param('fileId', IsStringNumberPipe) fileId: string, @Query() query: FindFileMetadataByFileIdBodyDto) {
  ...
  @Post('/')
  @Roles(Role.ADMIN)
  createMetadata(@TraceId() traceId: string | undefined, @Param('fileId', IsStringNumberPipe) fileId: string, @Body() body: CreateFileMetadataBodyDto) {
    const numberFileId = Number(fileId);
  ...
  @Delete('/:metadataId')
  @Roles(Role.ADMIN)
  deleteMetadataById(@TraceId() traceId: string | undefined, @Param('metadataId', IsStringNumberPipe) metadataId: string) {
