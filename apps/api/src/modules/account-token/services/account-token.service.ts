...
export class AccountTokenService implements Service {
  constructor(@Inject(ACCOUNT_TOKEN_REPOSITORY) private readonly repository: AccountTokenRepository, private readonly rmqErrorService: RmqErrorService, private readonly amqpConnection: AmqpConnection) { }
  ...
  async deleteTokens(accountId: number): Promise<AccountTokenModel[]> {
    ...
    await this.amqpConnection.publish<OnDeleteAccountTokensEvent.Request>(OnDeleteAccountTokensEvent.exchange, OnDeleteAccountTokensEvent.routingKey, { accountId });
