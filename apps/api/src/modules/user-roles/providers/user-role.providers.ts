...
export const userRoleServiceProvider: Provider = {
  ...
  useFactory: (repository: UserRoleRepository, amqpConnection: AmqpConnection, rmqResponseService: RmqResponseService, cognitoService: CognitoService, rmqErrorService: RmqErrorService) => {
    return new UserRoleService(repository, amqpConnection, rmqResponseService, cognitoService, rmqErrorService);
  },
  inject: [USER_ROLE_REPOSITORY, AmqpConnection, RmqResponseService, COGNITO_SERVICE, RmqErrorService],
};
