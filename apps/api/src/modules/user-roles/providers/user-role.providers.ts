...
export const userRoleServiceProvider: Provider = {
  ...
  useFactory: (repository: UserRoleRepository, amqpConnection: AmqpConnection, rmqResponseService: RmqResponseService, cognitoService: CognitoService) => {
    return new UserRoleService(repository, amqpConnection, rmqResponseService, cognitoService);
  },
  inject: [USER_ROLE_REPOSITORY, AmqpConnection, RmqResponseService, COGNITO_SERVICE],
};
