...
} from './workspace-user.service.interface';
...
} from '../repositories/workspace-user.repository';
...
export class WorkspaceUserService implements Service {
  constructor(
    ...
    private readonly repository: WorkspaceUserRepository,
  ...
  private async findWorkspaceRole(role: WorkspaceRole, traceId?: string) {
    ...
    const response =
      await this.amqpConnection.request<FindWorkspaceRoleQuery.Response>({
  ...
  private async update(
    ...
    input: Partial<WorkspaceUserModel>,
  ) {
