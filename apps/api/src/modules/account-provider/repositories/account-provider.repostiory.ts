...
export class AccountProviderRepository {
  ...
  async createOne(input: InsertAccountProvider) {
    return this.db.insert(this.accountProviders).values(input).returning().execute();
  }
  ...
  async updateByKey(key: ProviderKey, input: Partial<InsertAccountProvider>) {
    ...
    const result = await this.db.update(this.accountProviders).set(input).where(where).returning().execute();
  ...
  async deleteByKey(key: ProviderKey) {
    ...
    const result = await this.db.delete(this.accountProviders).where(where).returning().execute();
